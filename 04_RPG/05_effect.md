...
@override
  Future<void> onLoad() async {
    ...
    Particle _noopParticle() =>
        ComputedParticle(lifespan: 0, renderer: (_, __) {});

    // ⭐️修正　関数化
    Particle _buildByType() {
      switch (data.type) {
        case EffectType.smallExplosion:
          return createExplosion(100);
        case EffectType.mediumExplosion:
          return createExplosion(150);
        case EffectType.largeExplosion:
          return createExplosion(250);
        case EffectType.attackNormal:
          return createLineBurst(200);
        case EffectType.attackCritical:
          return createLineBurst(180);
        case EffectType.hitSpark:
          return createHitSpark();

        // スプライト系はこのクラスでは使わないので no-op を返す
        case EffectType.spriteExplosion:
        case EffectType.spriteSlash:
        case EffectType.spriteMagic:
          return _noopParticle();
      }
    }

    // ⭐️追加
    if (data.delaySec > 0) {
      // 指定秒待ってから particle をセット
      add(
        TimerComponent(
          period: data.delaySec,
          removeOnFinish: true,
          onTick: () {
            particle = _buildByType();
          },
...
class SpriteEffect extends SpriteAnimationComponent with HasGameRef<MainGame> {
  ...
  Future<void> onLoad() async {
    ...
    //⭐️ animation = SpriteAnimation(frames, loop: false);

    //⭐️
    final anim = SpriteAnimation(frames, loop: false);
    //⭐️
    void _start() {
      animation = anim;
      _started = true;
